...
\title{On data and medatata formats for electrophysiological experiments}


% author names and affiliations
% use a multiple column layout for up to three different
% affiliations
\author{\IEEEauthorblockN{Jiří Vaněk, Roman Mouček}
\IEEEauthorblockA{Department of Computer Science and Engineering, Faculty of Applied Sciences,\\
...
Electrophysiology data in our neuroinformatics laboratory are recorded using the BrainVision Recorder \cite{brainvision} and stored into three files. The format of these files (BrainVision file forma)is described in the BrainVision Recorder User Manual \cite{brainUserManual}:
\begin{itemize}
...
\begin{figure}
	\begin{center}
		\includegraphics[scale=0.51]{obrazky/data.eps}
...
\begin{itemize}
	\item data and metadata from locally stored BrainVision files are only exported,
...
Several file size tests were conducted to determine the resulting file size of the HDF5 container storing all data. Data from real experiments were used for these tests. The test showed that the size of the HDF5 container was influenced by exported data and the original file size was not the only decisive factor. The size of the HDF5 container was always bigger than original BrainVision files, four times bigger at the worst case, two times at the best case. The results are shown in Table~\ref{file_size} and Table~\ref{chunk_size}.
